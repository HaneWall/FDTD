...
# BUILD SETUP
indices = [81 + i for i in np.arange(0, 40, 1)]
...
for ind in indices:
    ...
    # Step 3: init sources
    test[30] = f.ActivatedSinus(name='sin**2acivation', wavelength=400.0e-09, carrier_wavelength=10000.0e-9, phase_shift=0, amplitude=1)
    ...
    # Step 4: add observer
    test[190] = f.QuasiHarmonicObserver(name='firstobserver', first_timestep=1800)
...
axes[0][0].plot(np.array(indices)-50, np.array(theo_amplitude), label='Theo')
axes[0][0].grid(True, linestyle='-.', alpha=0.3)
axes[0][0].plot(np.array(indices)-50, np.array(exp_amplitude))
axes[0][0].legend()
axes[0][0].set_xlabel('Breite des Mediums in '+r'$\Delta_x$')
axes[0][1].plot(np.array(indices)-50, np.array(theo_amplitude)/np.array(exp_amplitude))
axes[0][1].set_xlabel('Breite des Mediums in '+r'$\Delta_x$')
axes[0][1].grid(True, linestyle='-.', alpha=0.3)
axes[1][0].plot(np.array(indices)-50, theo_phasenunterschied, label='Theoretischer Phasenuntserschied')
axes[1][0].set_xlabel('Breite des Mediums in '+r'$\Delta_x$')
axes[1][0].grid(True, linestyle='-.', alpha=0.3)
axes[1][0].plot(np.array(indices)-50, -np.array(exp_phase), label='Gemessene Phase')
axes[1][0].legend()
axes[1][1].set_xlabel('Breite des Mediums in '+r'$\Delta_x$')
axes[1][1].plot(np.array(indices)-50, -np.array(exp_phase) - np.array(theo_phasenunterschied))
...
# Step 2: init media
#test[100:145] = f.NonDispersiveMedia(name='Media4Epsleft', permeability=1, permittivity=2, conductivity=0)

# Step 3: init sources
test[20] = f.ActivatedSinus(name='sin**2acivation', wavelength=100.0e-09, carrier_wavelength=10000.0e-9, phase_shift=0, amplitude=1)
#test[20] = f.EnvelopeSinus(name='test', wavelength=200.0e-09, fwhm=600.e-09, amplitude=1, phase_shift=0, peak_timestep=300)
#test[30] = f.GaussianImpulse(name='test', amplitude=1, peak_timestep=300, fwhm=200.e-09, tfsf=True)
#test[20] = f.SinusoidalImpulse(name='test', amplitude=1, phase_shift=0, wavelength=70.e-09)
# Step 4: add observer
