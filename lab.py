...
#test[100] = f.EnvelopeSinus(name='EnevelopedSinusMitte', wavelength=200e-09, phase_shift=0, amplitude=1, fwhm=600e-09, peak_timestep=200)
#test[50] = f.GaussianImpulse(name='a ndererImpuls', amplitude=1, peak_timestep=60, fwhm=100.e-09)
test[34] = f.ActivatedSinus(name='sin**2acivation', wavelength=900.0e-09, carrier_wavelength=20000.0e-9, phase_shift=0, amplitude=1)
...
axes.plot(y, test.local_observers[0].amplitude * np.cos(test.sources[0].omega * test.dt * y + test.local_observers[0].phase), label='reconstructed', linestyle='dashed')
